...
public class RedisConfiguration {
    ...
    //如果只使用StringRedisTemplate, 只需要导入redis starter,并在配置文件中配置上 spring.redis 相关的参数即可。
    //但是此时无法使用redisTemplate, 如果想使用redisTemplate, 必须进行手工配置后才能使用
    //配置一个key为string,value为object的redisTemplate
    @Bean
